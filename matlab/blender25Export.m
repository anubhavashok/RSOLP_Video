...
fprintf(fid, 'camdata.lens_unit = ''FOV''\n');
...
% Define set uv function
fprintf(fid, 'def setUV(me, vals):\n');
fprintf(fid, '    for i in range(len(vals)):\n');
fprintf(fid, '        me.uv_layers[0].data[i].uv = vals[i]\n');
fprintf(fid, '    return\n');
...
for i=1:6    
    ...
    fprintf(fid, 'setUV(me, [(0,0), (0,1), (1,1), (1,0)])\n');
    fprintf(fid, '\n');
end
fprintf(fid, '\n');
...
for i=1:numel(scene.surfs3d)
    ...
    fprintf(fid, 'me.materials.append(mat)\n');
    fprintf(fid, 'uvTex = me.uv_textures.new(name+''UVTex'')\n');
    fprintf(fid, 'setUV(me, [');
    for j=1:numel(uvstr)
        fprintf(fid, '%s', strtrim(uvstr{j}));
    end
    fprintf(fid, '])\n');
end
...
%display(sprintf('cd %s; %s -b -P %s', out_dir, BLENDER_BIN, [scene.name '.py']))
[result,foo] = system(sprintf('cd %s; %s -b -P %s', out_dir, BLENDER_BIN, [scene.name '.py']));
